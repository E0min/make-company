...
import { Terminal, X, Activity, Loader2, AlertCircle, Star, UserCircle } from "lucide-react";
import type { AgentProfile, StructuredEvent, TeamDefinition } from "@/lib/types";
            ...
            <div className="flex flex-col gap-0.5 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="truncate text-sm font-bold">
                  {profile?.agent?.name ?? agentId}
                </CardTitle>
              </div>
              {profile?.agent?.name && (
                <span className="text-[10px] font-mono text-muted-foreground ml-[18px]">
                  {agentId}
                </span>
              )}
            </div>
            ...
            <>
              ...
              {profile?.agent?.team && (
                <div className="text-[11px] text-muted-foreground">
                  팀: <span className="text-foreground/80">
                    {teams?.[profile.agent.team]?.label ?? profile.agent.team}
                  </span>
                </div>
              )}
